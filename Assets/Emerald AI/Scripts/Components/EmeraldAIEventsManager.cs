...
namespace EmeraldAI
{
    public class EmeraldAIEventsManager : MonoBehaviour
    {
        ...
        /// <summary>
        /// Clears the AI's target as well as all of its surrounding targets' targets.
        /// </summary>
        public void ClearAllSurroundingTargets()
        {
            Collider[] NearbyTargets = Physics.OverlapSphere(transform.position, EmeraldComponent.DetectionRadius, EmeraldComponent.DetectionLayerMask);
            for (int i = 0; i < NearbyTargets.Length; i++)
            {
                if (NearbyTargets[i].CompareTag(EmeraldComponent.EmeraldTag))
                {
                    NearbyTargets[i].GetComponent<EmeraldAIEventsManager>().ClearTarget();
                }
            }
        ...
        public void StopMovement()
        {
            ...
            EmeraldComponent.m_NavMeshAgent.velocity = Vector3.zero;
        }
        ...
        /// <summary>
        /// Removes the Faction from the AI's Faction Relations List. Note: The faction must exist within the Faction Manager's Current Faction List.
        /// </summary>
        /// <param name="Faction"> The name of the faction to remove.</param>
        public void RemoveFactionRelation(string Faction)
        {
            EmeraldAIFactionData FactionData = Resources.Load("Faction Data") as EmeraldAIFactionData;
            if (!FactionData.FactionNameList.Contains(Faction))
            {
            ...
            var FactionToRemove = EmeraldComponent.FactionRelationsList.Find(x => x.FactionIndex == FactionData.FactionNameList.IndexOf(Faction));
            EmeraldComponent.AIFactionsList.Remove(FactionData.FactionNameList.IndexOf(Faction));
            EmeraldComponent.FactionRelationsList.Remove(FactionToRemove);
        }
