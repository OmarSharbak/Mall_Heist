...
namespace MoreMountains.TopDownEngine
{
	...
	public enum TopDownEngineEventTypes
	{
		...
		PlayerDeath,
		[Tooltip(
			"(JayMountainsExtended) When the player dies, you have a chance to retry by either watching an ad or comsuming an item. " +
			"This is to accept death when given the Retry option.")]
		PlayerDeathAccepted,
		[Tooltip(
			"(JayMountainsExtended) When the player dies, you have a chance to retry by either watching an ad or comsuming an item. " +
			"This is different from TDE's Respawn which resets the level with various other triggers. To avoid complication added my own event.")]
		PlayerDeathRetry,
		SpawnComplete,
		RespawnStarted,
		RespawnComplete,
