...
{	
	...
	public class InventoryDisplay : MonoBehaviour, MMEventListener<MMInventoryEvent>
	{
		...
		[MMInformation("There is a bug with 'DrawEmptySlots' stated by Alexandru Gheorghe on Discord. " +
					   "When trying to make it look like the inventory is empty if there are no items (like a mobile card game), hiding empty slots doesn't work as intended. " +
					   "Added my own check with this bool to make it work.", MMInformationAttribute.InformationType.Info, false)]
		...
		/// </summary>
		public void RedrawInventoryDisplay()
		{
			InitializeSprites();
			AddGridLayoutGroup();
			DrawInventoryContent();		
		...
		public virtual void OnMMEvent(MMInventoryEvent inventoryEvent)
		{
			...
			switch (inventoryEvent.InventoryEventType)
			{
				...
				case MMInventoryEventType.ContentChangedRemoved:
					ContentHasChanged();
					break;

				case MMInventoryEventType.ContentChangedRemovedLast:
					ContentHasChanged();
					break;
