...
{	
	...
	/// </summary>
	public enum MMInventoryEventType
	{
		Pick,
		Select,
		Click,
		...
		Redraw,
		ContentChanged,
		[Tooltip("(JayMountainsExtended) Generally, 'ContentChanged' is used for inventory changes when item gets added/removed/moved that affects an inventory as a whole. Use this if you just want to update a slot while being in the same inventory.")]
		ContentChangedSlot,
		[Tooltip("(JayMountainsExtended) One item in a stack from an inventory was removed.")]
		ContentChangedRemoved,
		[Tooltip("(JayMountainsExtended) The last item from an inventory was removed.")]
		ContentChangedRemovedLast,
		InventoryOpens,
