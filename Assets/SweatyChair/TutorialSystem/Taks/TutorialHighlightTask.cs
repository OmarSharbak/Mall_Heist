    ...
    public class TutorialHighlightTask : TutorialShowPanelTask
    {
        ...
        protected virtual void RepositionHand()
        {
            if (showHand)
            {
                ...
                if (handFollowTarget)
                { // Calculate the hand position if follow target
                    if (isTargetUI)
                    {
        ...
        private void RepositionText()
        {
            if (showText)
            {
                ...
                if (textFollowTarget)
                { // Calculate the text position if follow target
                    if (isTargetUI)
                    {
        ...
        {
            if (targetTF != null && targetTF.gameObject.layer == LayerMask.NameToLayer("UI"))
            { // Only do for UI
                ...
                if (cloneTarget && _targetCloneGO == null)
                { // Position the target clone to target's position
