...
public class EscalatorManager : NetworkBehaviour
{
	...
	[SerializeField] private List<HackableATM> _hackableATMs;
	...
	void CalculateTotalMoney()
	{
		...
		for (int i = 0; i < _hackableATMs.Count; i++)
		{
			totalMoney += _hackableATMs[i].MoneyAmount;
		}
	...
	public void CheckExposed(PlayerState player)
	{
		...
		foreach (EmeraldAIEventsManager guard in allGuards)
		{
			...
			if (emeraldAIDetection.EmeraldComponent.CurrentTarget == player.playerTransform && emeraldAIDetection.EmeraldComponent.CurrentTarget != null)
				isExposed = true;
	...
	private void Update()
	{
		...
		if (playerLocal != null)
			CmdUpdateEscalatorOutLine(playerLocal);
		if (playerRemote != null)
			CmdUpdateEscalatorOutLine(playerRemote);
	...
	{
		if (allObjectivesComplete && (totalMoney == moneyCollected))
		{
	...
	[Command(requiresAuthority = false)]
	public void CmdUpdateEscalatorOutLine(PlayerState player)
	{
		UpdateEscalatorOutLine(player);
	}
	...
	public void UpdateEscalatorOutLine(PlayerState player)
	{
		...
		if (allObjectivesComplete && (totalMoney == moneyCollected)) // && allDoorsSealed
		{
	...
	[ServerCallback]
	public void CheckAllObjectivesComplete()
	{
