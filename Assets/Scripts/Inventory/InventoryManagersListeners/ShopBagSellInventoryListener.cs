...
{
    public class ShopBagSellInventoryListener : MMSingleton<ShopBagSellInventoryListener>, MMEventListener<MMInventoryEvent>
    {
        public InventoryDisplay ShopBagSell_InventoryDisplay;

        [SerializeField] private ShopBagSellItem shopBagSellItem;
        [SerializeField] private GameObject sellConfirmationDisplay;

        [ShowInInspector, ReadOnly] private Inventory bag = null;
        [ShowInInspector, ReadOnly] private Inventory bagSell = null;
        ...
        protected override void Awake()
        {
            ...
            foreach (Inventory inventory in UnityEngine.Object.FindObjectsOfType<Inventory>())
            {
                ...
                if ((inventory.name == "ShopBagSell_Inventory"))
                {
                    bagSell = inventory;
                }
            }
        ...
        private void GetSellableItems()
        {
            bagSell.EmptyInventory();

            for (int i = 0; i < bag.Content.Length; i++)
            {
                if (bag.Content[i] != null)
                {
                    if (((BagItem)bag.Content[i]).CanSell == true)
                    {
                        bagSell.AddItem(bag.Content[i], bag.Content[i].Quantity);
                    }
                }
            }
        }
        ...
        {
            GetSellableItems();
            ShopBagSell_InventoryDisplay.RedrawInventoryDisplay();
        }
        ...
        public void OnMMEvent(MMInventoryEvent eventType)
        {
            if (eventType.InventoryEventType == MMInventoryEventType.Click)
            {
                ...
                if (slot.ParentInventoryDisplay == ShopBagSell_InventoryDisplay)
                {
                    Debug.Log($"{slot.gameObject.name} was clicked.");
                    shopBagSellItem.SetItemData(slot, slot.ParentInventoryDisplay.TargetInventory, slot.Index);
